...
import 'package:Skynet/stories/story_1.dart';
import 'package:Skynet/util/story_bars.dart';



class StoryPage extends StatefulWidget {
  final String icon; // Определение переменной title
  final String imageUrl; // Определение переменной imageUrl
  const StoryPage({Key? key,required this.icon, required this.imageUrl}) : super(key: key);
...
class _StoryPageState extends State<StoryPage> {
  ...
  List<Widget> get myStories {
    // Здесь можно добавить несколько историй, если нужно
    return [
      MyStory1(icon: widget.icon, imageUrl: widget.imageUrl),
    ];
  }
